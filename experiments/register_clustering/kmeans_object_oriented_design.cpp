...
//ClusterOOD *FlipFlopF::clusterBest() const
//{
//    return clusterBest_;
//}

//void FlipFlopF::setClusterBest(ClusterOOD *clusterBest)
//{
//    clusterBest_ = clusterBest;
//}

//geometry::Point FlipFlopF::position() const
//{
//    return position_;
//}

//void FlipFlopF::setPosition(const geometry::Point &position)
//{
//    position_ = position;
//}
...
void ClusterOOD::insertElement(FlipFlop *element)
{
...
//-------------------------  Class Cluster EXT-------------------------
ophidian::experiments::register_clustering::ClusterOODext::ClusterOODext(const ophidian::geometry::Point &center) : center_(center)
{

}

std::vector<FlipFlopExt *> ClusterOODext::elements() const
{
    return elements_;
}

void ClusterOODext::elements(const std::vector<FlipFlopExt *> &clusterElements)
{
    elements_ = clusterElements;
}


geometry::Point ClusterOODext::center() const
{
    return center_;
}

void ClusterOODext::center(const geometry::Point &clusterCenter)
{
    center_ = clusterCenter;
}

void ClusterOODext::insertElement(FlipFlopExt *element)
{
    elements_.push_back(element);
}

void ClusterOODext::clear()
{
    elements_.clear();
}

int ClusterOODext::size()
{
    return elements_.size();
}
...
{
    clusters_.reserve(k);
    clustersExt_.reserve(k);
    for (int i = 0; i < k; ++i)
    {
        clusters_.push_back(ClusterOOD(geometry::Point(m_distribution_x(m_generator), m_distribution_y(m_generator))));
        clustersExt_.push_back(ClusterOODext(geometry::Point(m_distribution_x(m_generator), m_distribution_y(m_generator))));
    }
}
...
KmeansObjectOrientedDesign::KmeansObjectOrientedDesign(const std::vector<geometry::Point> &centers)
{
    ...
    clustersExt_.reserve(centers.size());
    for(auto p : centers)
    {
        clusters_.push_back(ClusterOOD(p));
        clustersExt_.push_back(ClusterOODext(p));
    }
}


void KmeansObjectOrientedDesign::cluster_registers_with_rtree(std::vector< std::unique_ptr<FlipFlop> > &flip_flops, ophidian::experiments::Metric &metric, unsigned iterations)
{
#if 0
...
//             << "\n expected cache misses: " << design.netlist().size(ophidian::circuit::Cell()) * sizeof(ophidian::experiments::chip_boundaries::PlacementCell) / 64
    ...
    for (int i = 0; i < iterations; ++i)
    {
        ...
        {
            FlipFlop & flip_flop = **flip_flop_it;
            closest_nodes.clear();
...
void KmeansObjectOrientedDesign::cluster_registers_with_rtree(std::vector< FlipFlopExt > &flip_flops, ophidian::experiments::Metric &metric, unsigned iterations)
{
#if 0
    std::cout<<"---- INFO: teste miss----"
             <<"\nNumber flipflops: "<< flip_flops.size()
...
//             << "\n expected cache misses: " << design.netlist().size(ophidian::circuit::Cell()) * sizeof(ophidian::experiments::chip_boundaries::PlacementCell) / 64
    ...
    for (int i = 0; i < iterations; ++i)
    {
        rtreeExt clusters_rtree;
        for (ClusterOODext & cluster : clustersExt_)
        {
            clusters_rtree.insert(rtree_nodeExt(cluster.center(), &cluster));
            cluster.clear();
        }
        std::vector<rtree_nodeExt> closest_nodes;
        for(auto flip_flop_it = flip_flops.begin(); flip_flop_it != flip_flops.end(); ++flip_flop_it)
        {
            FlipFlopExt & flip_flop = *flip_flop_it;
            closest_nodes.clear();
            clusters_rtree.query(boost::geometry::index::nearest(flip_flop.position(), 1), std::back_inserter(closest_nodes));
            ClusterOODext * closest_cluster = closest_nodes.front().second;
            closest_cluster->insertElement(&flip_flop);
        }
        for(auto cluster_it = clustersExt_.begin(); cluster_it != clustersExt_.end(); ++cluster_it)
        {
            auto cluster = *cluster_it;
            if(cluster.elements().size() != 0)
            {
                double x_c = 0, y_c = 0;
                for(FlipFlopExt * p : cluster.elements())
                {
                    x_c += p->position().x();
                    y_c += p->position().y();
                }
...
void KmeansObjectOrientedDesign::cluster_registers_with_rtree_paralel(std::vector< std::unique_ptr<FlipFlop> > &flip_flops, ophidian::experiments::Metric &metric, unsigned iterations)
{
    metric.start();
    for (int i = 0; i < iterations; ++i)
    {
